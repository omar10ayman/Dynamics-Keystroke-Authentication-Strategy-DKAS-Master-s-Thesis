...
The proposed architecture combines **ResNet1D** for extracting local temporal patterns, **Bidirectional LSTM (BiLSTM)** for modeling long-term dependencies, and a **Multi-Head Attention** mechanism to focus on the most informative keystroke features. To improve robustness and generalization, the model employs **Mixup data augmentation** and **cross-entropy loss with label smoothing**.

For model interpretability, **Grad-CAM** is integrated to visualize the temporal features that contribute most to authentication decisions, providing transparent and explainable predictions.

## Highlights
- ResNet1D for temporal feature extraction
